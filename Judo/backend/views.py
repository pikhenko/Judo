...
menu = [
        ...
        {'title': "Стоимость", 'url_name': 'backend:price'},
...
def price(request):
    ...
    return render(request, 'backend/price.html', context=context)
...
def index(request):
    posts_list = Posts.objects.order_by('time_create') # [:2]
    ...
    if schedules.exists():
        ...
        message = f'Привет {request.user.username}, сегодня нет тренировок! Можно отдохнуть.' \
            if request.user.is_authenticated else 'Добро пожаловать!'
        messages.add_message(request, messages.INFO, message)
...
def team(request):
    ...
    return render(request, 'backend/team.html', context=context)
